...
export default React.createClass({
    render() {
    	...
    	var shiftRef = new Firebase('https://schedulr-c0fd7.firebaseio.com/shifts/mannies/'+date+'/'+rowMap[time]);
    	...
    	if(employeeAssigned.length > 0){
    		return (
				<td className={'time-slot-eile ' + cellID}>  			
					<button> 
						<span id='message-'>{employeeAssigned}</span> 
					</button> 
				</td> 
	        )
    	}else{
    		return (
				<td id={'cell-'+cellID} className={'time-slot'}> 
